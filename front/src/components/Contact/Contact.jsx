import "./Contact.css";
import { useContext, useEffect } from "react";
import { FixedContext } from "../../contexts/FixedContext";
import RegisterForm from "../Homepage.jsx/RegisterForm";

function Contact() {
  const { fix, setFix } = useContext(FixedContext);

  useEffect(() => {
    setFix(false);
  }, []);

  return (
    <div id="contact-container">
        <br />
      <div className="contact-title">
        <h1 id="contact-header">צור קשר</h1>
      </div>
      <div id="contact-row">
        <div>
          <div id="contact-links">
            <h2 className="contact-small-header"> כתבו לנו</h2>
            <div className="contact-link-section">
              <a
                className="icon-container"
                href="https://api.whatsapp.com/send?phone=972502449054&text=שלום,%20ארצה%20לשמוע%20עוד%20על%20הכנס%20שיקרה%20ב-2.8"
                target="_blank"
                rel="noopener noreferrer"
              >
                <img src="data:image/png;base64," />
              </a>
              <a
                href="https://api.whatsapp.com/send?phone=972502449054&text=שלום,%20ארצה%20לשמוע%20עוד%20על%20הכנס%20שיקרה%20ב-2.8"
                target="_blank"
                rel="noopener noreferrer"
              >
                <h2 id="contact-phone-text">050-244-9054</h2>
              </a>
            </div>
          </div>
          <div className="contact-link-section">
            <a
              className="icon-container"
              href="http://"
              target="_blank"
              rel="noopener noreferrer"
            >
              <img
                width={40}
                src="data:image/png;base64,"
              />
            </a>
            <a
              className="icon-container"
              href="mailto: Shanigal@cyberpro-israel.co.il?subject=%D7%9B%D7%A0%D7%A1%20%D7%91%D7%95%D7%92%D7%A8%D7%99%D7%9D%20%D7%A1%D7%99%D7%99%D7%91%D7%A8%D7%A4%D7%A8%D7%95%202.8&body=%D7%A9%D7%9C%D7%95%D7%9D%20%D7%A9%D7%A0%D7%99%2C%0D%0A%D7%90%D7%A0%D7%99%20%D7%9E%D7%A2%D7%95%D7%A0%D7%99%D7%99%D7%9F%20%D7%9C%D7%A7%D7%91%D7%9C%20%D7%9E%D7%99%D7%93%D7%A2%20%D7%A2%D7%9C%20%D7%94%D7%9B%D7%A0%D7%A1%20%D7%94%D7%A7%D7%A8%D7%95%D7%91."
            >
              <h2 id="contact-email-text">Shanigal@cyberpro-israel.co.il</h2>
            </a>
          </div>
        </div>
        <div>
          <div id="contact-travel">
            <h2 className="contact-small-header">דרכי הגעה</h2>
            <div id="contact-travel-icons">
              <a
                className="icon-container"
                id="link-adress-a"
                href="https://www.waze.com/he/live-map/directions?to=ll.32.071474%2C34.789345"
              >
                <img
                  width={60}
                  src="data:image/png;base64,"
                ></img>
              </a>
              <a
                className="icon-container"
                href="https://www.google.com/maps/place/Derech+Menachem+Begin+121,+Tel+Aviv-Jaffa/@32.0718027,34.7890235,17z/data=!4m6!3m5!1s0x151d4b9c46209285:0x4aa8c0e33c2116d8!8m2!3d32.0718027!4d34.7890235!16s%2Fg%2F11gbx9_6th"
              >
                <img src="data:image/png;base64,"></img>
              </a>
            </div>
          </div>
        </div>
      </div>
      <RegisterForm />
      <iframe
        src="https://www.google.com/maps/d/embed?mid=1kAGy6WRb-0mp0q7Gce9GCSuYR_DzPYc&ehbc=2E312F"
        width="100%"
        height="480"
      ></iframe>
    </div>
  );
}

export default Contact;
